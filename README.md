...
<style>
/* ═══ VARIABLES — RED · WHITE · BLACK ═══ */
:root{
  --bg:#0d0000;--bg2:#1a0000;--sur:#220000;--sur2:#2e0000;--bdr:#550000;
  --f1:#cc0000;--f2:#e00000;--f3:#ff2222;--f4:#ff6666;
  --txt:#ffffff;--txt2:#ffaaaa;--txt3:#884444;
  --hl:rgba(220,0,0,.25);--hlw:#ff4444;
  --r:12px;--r2:20px;--page-w:calc(100vw - 32px);
  --gfire:linear-gradient(135deg,#8b0000,#cc0000 45%,#ff2222);
  --gshadow:0 8px 32px rgba(180,0,0,.3);
  --theme-color:#0d0000;
}
...
.quote-wrap{padding:22px 20px;background:linear-gradient(180deg,#1a0000,#0d0000);
...
.cta-bg{position:absolute;inset:0;border-radius:var(--r2);
  background:linear-gradient(135deg,#3a0000,#880000,#cc0000,#ff2222);
  animation:cpulse 2.8s ease-in-out infinite}
@keyframes cpulse{0%,100%{filter:brightness(1)}50%{filter:brightness(1.15)}}
...
.cta-label{font-family:'DM Serif Display',serif;font-size:21px;font-weight:900;color:#fff;
  text-shadow:0 2px 8px rgba(80,0,0,.9);letter-spacing:.5px}
...
.lib-del{width:36px;height:36px;border-radius:9px;border:1px solid rgba(200,0,0,.35);
  background:rgba(200,0,0,.1);color:var(--f3);font-size:16px;cursor:pointer;
  display:flex;align-items:center;justify-content:center;flex-shrink:0;
  transition:all .15s}
.lib-del:active{background:rgba(200,0,0,.3);border-color:var(--f2)}
...
/* Book pages scroll area — fills all remaining space */
.book-scroll{
  flex:1;
  overflow-y:auto;
...
.book-page{
  background:#ffffff;border-radius:4px 4px 2px 2px;
  box-shadow:0 2px 0 #ccc,0 4px 0 #bbb,0 6px 0 #aaa,
             0 8px 20px rgba(0,0,0,.6);
  margin-bottom:28px;position:relative;overflow:hidden;
  border:1px solid #ddd;
}
/* Page top bar — white with red accent */
.page-num-bar{
  display:flex;align-items:center;justify-content:space-between;
  padding:8px 18px 6px;border-bottom:1px solid #e0e0e0;
  background:#fff;
}
.page-num-label{font-family:'JetBrains Mono',monospace;font-size:11px;
  color:#cc0000;font-weight:700;letter-spacing:.5px}
.page-doc-name{font-size:10px;color:#666;font-style:italic;
  white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:55%}
/* Page content — pure white, pure black text */
.page-content{padding:20px 22px 24px;min-height:120px;background:#fff}
.page-content p{
  font-family:'DM Serif Display',serif;font-size:17px;
  line-height:1.88;margin-bottom:1.2em;
...
.page-content .word:hover{background:rgba(200,0,0,.12)}
.page-content .word.speaking{
...
.page-foot{
  display:flex;align-items:center;justify-content:center;
  padding:6px 18px 10px;border-top:1px solid #e0e0e0;
  background:#fff;
}
.page-foot-num{font-family:'JetBrains Mono',monospace;font-size:11px;color:#999}
/* Page spine */
.book-page::before{
...
@keyframes mpulse{0%{box-shadow:0 0 0 0 rgba(200,0,0,.5)}
  70%{box-shadow:0 0 0 18px rgba(200,0,0,0)}100%{box-shadow:0 0 0 0 rgba(200,0,0,0)}}
...
/* ═══ MISC ═══ */
.spin{width:17px;height:17px;border:2px solid rgba(200,0,0,.2);
...
<!-- ════════════════════════════════
  READER
════════════════════════════════ -->
<div class="page" id="page-reader" style="display:flex;flex-direction:column">
  <div class="nav">
  ...
  <div id="readerEmpty" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:32px;text-align:center">
    ...
    <div style="font-family:'DM Serif Display',serif;font-size:22px;color:#fff">Nothing loaded yet</div>
  ...
  <!-- Reader main — hidden until doc loaded -->
  <div id="readerMain" style="display:none;flex-direction:column;overflow:hidden;min-height:0">
    <div class="reader-tb">
      <div class="font-btns">
  ...
  <div class="playbar" id="playbar" style="display:none">
    <div class="lock-badge" id="lockBadge"><span>🔒 AUTO-RESUMES WHEN UNLOCKED</span></div>
    <div class="pb-row">
...
function spawnEmbers(){
  ...
  const cols=['#cc0000','#e00000','#ff2222','#ff5555','#ffffff'];
  for(let i=0;i<20;i++){
...
function delDoc(id){
  // No confirm dialog — direct delete with undo toast
...
function loadDoc(doc){
  ...
  rm.style.minHeight='0'; // critical for flex children to scroll correctly
...
function startSpeechFrom(idx){
  ...
  utterance.onboundary=e=>{
    ...
    if(words[li]){
      ...
      words[li].el.style.background='#cc0000';
      words[li].el.style.color='#ffffff';
      words[li].el.scrollIntoView({behavior:'smooth',block:'center'});
      wordIdx=li;
      updateProgress();
      updatePageIndicator();
      updateMediaSession();
...
function clearHL(){
  words.forEach(w=>{
    w.el.classList.remove('speaking');
    w.el.style.color='#111111';
...
function updateProgress(){
  ...
  if(currentDoc)currentDoc.progress=Math.round(pct);
}
...
function changeFontSize(d){
  ...
  // Directly set font-size on every paragraph inside book pages
...
function animateWave(){
  ...
  const cols=['#8b0000','#cc0000','#e00000','#ff2222'];
  for(let i=0;i<26;i++){
...
function stopWaveAnim(){
  ...
  const c=['#8b0000','#cc0000','#e00000','#ff2222'];
...
document.addEventListener('visibilitychange',async()=>{
  if(document.visibilityState==='hidden'){
    // Remember whether we were mid-read the moment the screen went away —
    // covers both auto-timeout and a manual power-button lock.
    wasPlayingWhenHidden=isPlaying;
    return;
  }
  if(!isPlaying)return;
  await requestWakeLock();
  // iOS reports stale synth.paused/synth.speaking values for a beat right
  // after waking — give it a moment before deciding what actually happened.
  setTimeout(()=>{
    if(!isPlaying)return;
    if(synth.paused){
      // A genuine soft-pause (rare, but happens) — just resume it.
      synth.resume();
      updateMediaSession();
    }else if(!synth.speaking){
      // iOS suspended the speech engine while locked. Restart cleanly from
      // the last word that was confirmed speaking, no manual tap needed.
      startSpeechFrom(wordIdx);
      if(wasPlayingWhenHidden)showToast('🔥 Resumed where you left off');
    }
  },250);
});
...
function saveStorage(){
  try{
    localStorage.setItem('fr_lib',JSON.stringify(library.map(d=>({...d,text:d.text.slice(0,65000)}))));
